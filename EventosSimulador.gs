 ...
 *  - compasso_assertHomologacao_  responde "aqui é homologação?";
 *  - exigirAdminOuSessao_         responde "quem está chamando?".
 *
 * A primeira sozinha deixava qualquer visitante anônimo do web app disparar
 * 2.500 inscrições falsas no Firestore de homologação — e como não há prefixo
 * de coleção por ambiente, se o projeto apontar para o mesmo Firebase de
 * produção, a massa cai no acervo real. Exige administrador.
 */
...
function compasso_simulacaoIniciar(quantidade, tokenSessao) {
  exigirAdminOuSessao_(tokenSessao, 'eventos', 'Compasso — iniciar simulação', true);
...
function compasso_simulacaoExecutarLote(loteId,tamanhoLote,tokenSessao) {
  exigirAdminOuSessao_(tokenSessao, 'eventos', 'Compasso — executar lote da simulação', true);
  ...
  for(var i=Number(s.proximo);i<=fim;i++){
    ...
    var ri=compasso_criarInscricaoAssociado({nome:'SIMULADO '+String(i).padStart(4,'0'),cpf:cpf,pessoaId:pessoaId,escola:'ESCOLA SIMULADA '+((i%20)+1),cidade:'CIDADE '+((i%5)+1),origem:'SIMULADOR'},tokenSessao);
    ...
    try{compasso_validarDecisaoAdmin(ri.inscricaoId,COMPASSO_STATUS.VALIDADA,'','',tokenSessao);s.validados=Number(s.validados||0)+1;var re=compasso_emitirIngressoV2({inscricaoId:ri.inscricaoId},tokenSessao);if(re.ok)s.emitidos=Number(s.emitidos||0)+1;else s.erros=Number(s.erros||0)+1;}catch(e){s.erros=Number(s.erros||0)+1;}
  }
...
function compasso_simularMassa(quantidade, tokenSessao) {
  exigirAdminOuSessao_(tokenSessao, 'eventos', 'Compasso — simular massa', true);
  quantidade=Number(quantidade);var ini=compasso_simulacaoIniciar(quantidade,tokenSessao);
  if(quantidade<=50)return compasso_simulacaoExecutarLote(ini.loteId,50,tokenSessao);
  return ini;
}

function compasso_testeDuplicidade(cpf, tokenSessao) {
  exigirAdminOuSessao_(tokenSessao, 'eventos', 'Compasso — teste de duplicidade', true);
  compasso_assertHomologacao_();cpf=cpf||String(Date.now()).slice(-11);var pessoaId='TEST-DUP-'+cpf;
  var a=compasso_criarInscricaoAssociado({nome:'TESTE DUPLICIDADE',cpf:cpf,pessoaId:pessoaId,origem:'SIMULADOR'},tokenSessao);
  var b=compasso_criarInscricaoAssociado({nome:'TESTE DUPLICIDADE',cpf:cpf,pessoaId:pessoaId,origem:'SIMULADOR'},tokenSessao);
  return {primeira:a,segunda:b,aprovado:!!a.ok&&!b.ok,erroEsperado:b.erro||''};
}

function compasso_testeQrReutilizado(qrToken,dispositivoA,dispositivoB,tokenSessao) {
  exigirAdminOuSessao_(tokenSessao, 'eventos', 'Compasso — teste de QR reutilizado', true);
  compasso_assertHomologacao_();var a=compasso_checkin_interno_(qrToken,dispositivoA||'CEL-A'),b=compasso_checkin_interno_(qrToken,dispositivoB||'CEL-B');
